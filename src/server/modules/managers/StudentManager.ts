...
class StudentManager extends BaseManager {
  ...
  constructor(options: StudentManagerOptions) {
    ...
    this.mediator.set(
      this.triggers.GET_STUDENTS_ON_LESSON,
      this.getStudentsOnLesson
    );
  }

  // Добавить студента data = {userId, groupId, type}
  setStudent = (data: { userId: number; groupId: number; type: number }) =>
    this.db.addStudent(data);

  setSocketIdToStudent = (student: User) => {
    const users = this.mediator.get(this.triggers.GET_ACTIVE_USERS);
    const userToken = Object.keys(users).find(
      (token: string) => users[token].id === student.id
    );
    if (userToken) {
        ...
        if (studentFromDB) {
          ...
          if (student && student.groupId === admin.groupId) {
            ...
            if (lesson) {
              ...
              const isStudentAlreadyHere = await this.db.getStudentOnLesson({
                studentId: student.id,
                lessonId: lesson.id,
              });
              if (!isStudentAlreadyHere) {
                // Отметить студента
                await this.db.noteStudent({
                  adminId: admin.id,
                  studentId: student.id,
                  lessonId: lesson.id,
                });
                // Получить список студентов на этой паре
                const students = await this.db.getStudentsOnLesson({
                  adminId: admin.id,
                  date: this.getDate(),
                  lessonId: lesson.id,
                });
  ...
  getStudentsOnLesson = async ({
    ...
    if (studentFromDB) {
      ...
      if (admin && (admin.type === 1 || admin.type === 2)) {
        ...
        if (lesson) {
          return this.db.getStudentsOnLesson({
            adminId: admin.id,
            date,
            lessonId: lesson.id,
          });
