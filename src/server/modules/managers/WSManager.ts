...
class WSManager {
    ...
    getStudentsList({ admin, students }: { admin: Student, students: Student[] }) {
        if (admin.socketId) {
    ...
    initSocket(socket: Socket) {
        ...
        if (me) {
            me.socketId = socket;
            this.io.local.emit(this.socketEvents.SEND_MESSAGE_TO_ALL, { text: `Пользователь ${me.name} подключился!`, id: me.id, name: me.name});
        }
    }
    ...
    getUser(users: { [token: string]: User }, socket: Socket) {
        const key = Object.keys(users).find((token: string) => users[token].socketId && (users[token].socketId.id === socket.id));
        return key && users[key];
    }

    private init() {
        this.mediator.subscribe(this.events.GET_STUDENTS_LIST, this.getStudentsList);
        this.io.on('connection', socket => {
            ...
            socket.on(this.socketEvents.SEND_MESSAGE, ({ text }) => {
                ...
                if (me) {
                    this.io.local.emit(this.socketEvents.SEND_MESSAGE_TO_ALL, { text, id: socket.id, name: me.name });
                }
            });
