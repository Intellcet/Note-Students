...
class DB {
  ...
  addUser = ({
    ...
    new Promise(resolve => {
      ...
      this.db.run(query, [login, password, name], err =>
        !err ? resolve(this.getUserByLogin(login)) : resolve(null)
      );
    });
  ...
  addStudent = ({
    ...
    new Promise(resolve => {
      const query =
        'INSERT INTO student (user_id, group_id, type) VALUES (?, (SELECT id FROM "group" WHERE "group".code = ?), ?)';
  ...
  getStudentByToken = (token: string): Promise<Student | null> =>
    new Promise(resolve => {
      const query =
        'SELECT student.id AS studentId, student.group_id AS groupId, student.type AS type, ' +
  ...
  getCurrentLesson = (): Promise<LessonTime> =>
    new Promise(resolve => {
      const query =
        "SELECT * FROM lesson_time WHERE time_start < time('now', 'localtime') " +
        "AND time('now', 'localtime') < time_finish";
  ...
  noteStudent = ({
    ...
    new Promise(resolve => {
      const query =
        ...
        "INSERT INTO student_on_lessons (admin_id, students_id, lesson_time_id, date, time) VALUES (?, ?, ?, date('now', 'localtime'), time('now', 'localtime'))";
  ...
  getStudentOnLesson = ({
    ...
    studentId: number;
    lessonId: number;
  }): Promise<StudentOnLesson | null> =>
    new Promise(resolve => {
      const query =
        'SELECT * FROM student_on_lessons' +
        "WHERE students_id = ? AND lesson_time_id = ? AND student_on_lessons.date = date('now', 'localtime')";
      this.db.get(query, [studentId, lessonId], (err, row) =>
  ...
  getStudentsOnLesson = ({
    ...
    new Promise(resolve => {
      const query =
        'SELECT student_on_lessons.date AS date, student_on_lessons.time AS time, ' +
        'lesson_time.num_lesson AS lessonNum, lesson_time.time_start AS timeStart, ' +
        'lesson_time.time_finish AS timeFinish, ' +
      ...
      this.db.all(query, [date, adminId, lessonId], (err, rows) =>
