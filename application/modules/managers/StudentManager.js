...
class StudentManager extends BaseManager {
    ...
    noteStudent = ({ tokenAdmin, tokenStudent }) => {
        const admin = new Student(this.db.getStudentByToken(tokenAdmin));
        if (admin && admin.type === 1 && admin.type === 2) {
            this.setSocketIdToStudent(admin);
            const student = new Student(this.db.getStudentByToken(tokenStudent));
            if (student && student.groupId === admin.groupId) {
                this.setSocketIdToStudent(student);
                const lesson = this.db.getCurrentLesson();
                if (lesson) {
                    // Смотрим, отметился ли уже этот студент на этой пару
                    const isStudentAlreadyHere = this.db.getStudentOnLesson({ studentId: student.id, lessonId: lesson.id});
                    if (!isStudentAlreadyHere) {
                        // Отметить студента
                        this.db.noteStudent({adminId: admin.id, studentId: student.id, lessonId: lesson.id});
                        // Получить список студентов на этой паре
                        const students = this.db.getStudentsOnLesson({ adminId: admin.id, date: this.getDate(), lessonId: lesson.id });
    ...
    getStudentsOnLesson = ({tokenAdmin, date, lessonNum }) => {
        const admin = new Student(this.db.getStudentByToken(tokenAdmin));
        if (admin && admin.type === 1 && admin.type === 2) {
            const lesson = this.db.getLessonByNum(lessonNum);
            if (lesson) {
